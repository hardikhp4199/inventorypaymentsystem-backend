...
contract InventoryPayment {
    ...
    event LogDebugMessage(string message);
    event LogOrderStatus(string message);
    ...
    function receiveOrder(uint256 _orderNo, OrderDetail[] memory _receivedProducts) external {
        Bank memory userBank;
        bool bankFound = false;
        uint256 userBankDetailsIndex=1;
        for (uint256 i = 1; i <= bankCount; i++) {
            if (bytes(userBankDetails[i].bankName).length != 0) {
                userBank = userBankDetails[i];
                userBankDetailsIndex = i;
                bankFound = true;
                break;
            }
        }
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            if (orders[i].orderNo == _orderNo && compareStrings(orders[i].orderStatus, "received")) {
                ...
                // Update user's bank details in the mapping
                userBankDetails[userBankDetailsIndex] = userBank;
                
                break;
            }
            emit LogDebugMessage("Stage 7");
        }
